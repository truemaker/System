uint8* graphics_back_buffer;//[GRAPHICS_MAX];
...
void gClear()
{
  for(uint32 index = 0; index < GRAPHICS_MAX; index++) {
    graphics_back_buffer[index] = 0;
  }
}
...
{
  for(uint32 index = 0; index < GRAPHICS_MAX; index++) {
    graphics_back_buffer[index] = color;
  }
}
void gSwap()
{
  for(uint32 index = 0; index < GRAPHICS_MAX; index++)
    graphics_buffer[index] = graphics_back_buffer[index];
}
